...
router.post('/users/register', async (req, res) => {
  try {
    ...
    if (nickname.length < 3 || check_spc.test(nickname) || check_kor.test(nickname)) {
    ...
    if (password.length < 4 || nickname.search(password) >= 0) {
      return res.status(400).send({
        success: false,
        message: '패스워드는 4자이상, 닉네임에 포함되지 않은 값으로 구성하여 주세요.',
      });
    }
...
router.post('/users/login', async (req, res, next) => {
  try {
    ...
    const token = jwt.sign({ userId: existUser.dataValues.id }, 'secretKey');
